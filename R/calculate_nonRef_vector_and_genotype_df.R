...
create_gt_dosages = function(vcf){
  ...
  variant_ids = paste0(vcf@fix[,"CHROM"],"_",vcf@fix[,"POS"],"_",vcf@fix[,"REF"])
  ...
  duplicated_rows = duplicated(variant_ids)
  gt = gt[!duplicated_rows,]
  message("Removing ", sum(duplicated_rows), " duplicated rows from vcf")

  ## removing those from the names
  variant_ids = variant_ids[!duplicated_rows]
  gt[, rn := variant_ids]
...
estimate_b_from_bam_readcount = function(bam_readcount,vcf){
    required_columns = c("TOTAL_READS", "A", "C", "G", "T", "CHROM_POS_REF")
    missing_columns = setdiff(required_columns, colnames(bam_readcount))
    if(length(missing_columns) != 0){
      stop("bam_readcount is missing required columns: ",
           paste(missing_columns, collapse = ", "))
    }
    if(!"ALT" %in% colnames(vcf)){
      stop("vcf must contain an ALT column.")
    }
...
calculate_nonRef_vector_and_genotype_df = function(bam_readcount_path,vcf_path,min_read_overlap = 1,
  ...
  if (sys_name == "Linux") {
    ...
    decompressed_path = tempfile(fileext = ".tsv")
    R.utils::gunzip(
      filename = bam_readcount_path,
      destname = decompressed_path,
      remove = FALSE,
      overwrite = TRUE
    )
    on.exit(unlink(decompressed_path), add = TRUE)
  ...
  message("Initial bam-readcount table has ", nrow(bam_readcount), " rows.")
  message("Summary of total reads per SNP:")
  print(summary(bam_readcount[["TOTAL_READS"]]))
  message("Coverage > 1 read:")
  print(table(bam_readcount[["TOTAL_READS"]] > 1))
  ...
  message("The proportion of rows with b_estimate from the input vcf was ",
          nrow(count_dt) / original_vcf_rows, " \n")
