...
#'
#' @param b A numeric vector of length equal to the number of SNPs retained for
#'   deconvolution.
#' @param A A data frame or data.table with one row per SNP and one donor column
#'   per sample, plus an `"rn"` column containing SNP identifiers. The number of
#'   rows of `A` must equal the length of `b`.
...
#' @export
estimate_weights <- function(b, A, force_zero = TRUE){
  if(missing(A)){
    stop("A genotype dosage table must be supplied via `A`.")
  }
  # check dimensions
  ...
  if(length(cols) == 0){
    stop("A must contain at least one donor column in addition to `rn`.")
  }
